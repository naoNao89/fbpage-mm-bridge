...
use crate::handlers::{
    create_attachment, create_message, get_attachment, get_attachments_by_message,
    get_message, get_messages_by_conversation, get_messages_by_customer, get_unsynced_messages,
    health_check, lookup_customer_by_conversation, mark_message_sync_failed, mark_message_synced,
...
pub fn create_app(state: AppState) -> Router {
    Router::new()
        ...
        .route("/api/attachments/message/:message_id", get(get_attachments_by_message))
        .route("/api/attachments/:id", get(get_attachment))
        .route("/api/attachments/:id/mm-file-id", put(update_attachment_mm_file_id))
        .with_state(state)
}
