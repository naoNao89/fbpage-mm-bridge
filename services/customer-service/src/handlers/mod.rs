...
use crate::db;
use crate::models::{CreateCustomerRequest, CustomerResponse, ListCustomersQuery, UpdateCustomerRequest};
use crate::AppState;
...
pub async fn list_customers(
    ...
    match db::list_customers(&state.pool, &query).await {
        Ok(customers) => {
            let responses: Vec<CustomerResponse> = customers.into_iter().map(|c| c.into()).collect();
...
pub async fn get_customers_without_mapping(
    State(state): State<AppState>,
) -> impl IntoResponse {
...
pub async fn get_customer_stats(
    ...
    match db::count_customers(&state.pool).await {
        Ok(total) => {
            ...
            let facebook_count = db::count_customers_by_platform(&state.pool, "facebook").await.unwrap_or(0);
            let zalo_count = db::count_customers_by_platform(&state.pool, "zalo").await.unwrap_or(0);
