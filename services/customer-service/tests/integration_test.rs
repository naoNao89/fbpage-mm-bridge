...
async fn test_get_customer_by_platform() {
    ...
    // Create a customer
    let customer = db::get_or_create_customer(&pool, "platform_test_user", "facebook", Some("Platform Test"))
        .await
        .unwrap();
...
async fn test_update_customer() {
    ...
    // Create a customer
    let customer = db::get_or_create_customer(&pool, "update_test_user", "facebook", Some("Original Name"))
        .await
        .unwrap();
